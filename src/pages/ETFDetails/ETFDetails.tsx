...
const ETFDetails: React.FC = () => {
    ...
    const loadETF = async () => {
        ...
        setError(null);
        try {
            const data = await ETFService.getETFDetails(id);
            if (data) setEtf(data);
    ...
    if (isLoading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-20">
                ...
                <p className="mt-4 text-indigo-900/60 font-bold tracking-widest uppercase">Loading ETF...</p>
            </div>
        );
    }
    ...
    if (error || !etf) {
        return (
            <div className="flex flex-col items-center justify-center p-10 h-[60vh]">
                <h2 className="text-2xl font-bold text-indigo-900 mb-4">{error || "ETF not found"}</h2>
    ...
    return (
        <PageShell className="animate-in fade-in slide-in-from-bottom-4 duration-700 overflow-x-hidden">
            ...
            {/* Key Statistics Grid */}
            <div className="mb-8 md:mb-12">
                <SectionTitle icon={<FiBarChart2 />} title="Key ETF Statistics" />
