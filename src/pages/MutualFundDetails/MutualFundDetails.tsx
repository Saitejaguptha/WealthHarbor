...
const MutualFundDetails: React.FC = () => {
    ...
    const loadFund = async () => {
        ...
        setError(null);
        try {
            const data = await MutualFundService.getMutualFundDetails(id);
    ...
    if (isLoading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-20">
                ...
                <p className="mt-4 text-indigo-900/60 font-bold tracking-widest uppercase">Loading fund details...</p>
            </div>
        );
    }
    ...
    if (error || !fund) {
        return (
            <div className="flex flex-col items-center justify-center p-10 h-[60vh]">
                <h2 className="text-2xl font-bold text-indigo-900 mb-4">{error || "Mutual Fund not found"}</h2>
                ...
                <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-indigo-50 shadow-xl shadow-indigo-50 p-5 sm:p-8 min-w-0 max-w-full">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-black text-indigo-950">{fund.fundManager?.name || 'Assigned Fund Manager'}</h3>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div className="flex gap-3">
                                    ...
                                    <p className="text-sm font-medium text-indigo-900/70">{fund.fundManager?.experience || 'Experience data unavailable'}</p>
                                </div>
                                <div className="flex gap-3">
                                    <FiPercent className="text-indigo-400 shrink-0 mt-1" />
                                    <p className="text-sm font-medium text-indigo-900/70">{fund.fundManager?.education || 'Educational details unavailable'}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {fund.fundManager?.otherFunds?.map((f: string, i: number) => (
                ...
                <div className="bg-indigo-900 rounded-[2.5rem] p-5 sm:p-8 text-white shadow-2xl shadow-indigo-100 flex flex-col justify-between min-w-0 max-w-full overflow-hidden">
                    <div>
                        ...
                        <div className="space-y-4">
                            {fund.schemeDocuments.map((doc: {name: string; url: string}, i: number) => (
