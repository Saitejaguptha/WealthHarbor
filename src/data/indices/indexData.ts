...
const generateIndices = (): MarketIndex[] =>
    rawIndices.map(idx => {
        ...
        return {
            ...idx,
            exchange: idx.exchange as 'NSE' | 'BSE',
...
export const INDIAN_INDICES = generateIndices();

export const getIndicesByExchange = (exchange?: 'NSE' | 'BSE'): MarketIndex[] => {
    if (!exchange) return INDIAN_INDICES;
    return INDIAN_INDICES.filter(idx => idx.exchange === exchange);
};

export const getIndexByName = (name: string): MarketIndex | undefined =>
    INDIAN_INDICES.find(idx => idx.name === name);
