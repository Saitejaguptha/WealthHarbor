    ...
    return (
        <header className="h-[60px] bg-white/95 backdrop-blur-sm flex items-center justify-between gap-2 px-3 sm:px-4 md:px-6 border-b border-indigo-100 shadow-sm z-50 fixed top-0 left-0 right-0 min-w-0">
            <div className="flex items-center gap-2 sm:gap-4 shrink-0 min-w-0">
                <button
                    onClick={onMenuClick}
                    className="p-2 -ml-1 sm:-ml-2 hover:bg-indigo-50 rounded-lg text-indigo-600 md:hidden transition-colors shrink-0"
                ...
                <Link
                    to="/"
                    className="text-base sm:text-lg md:text-xl font-bold text-indigo-900 tracking-tight hover:text-indigo-600 transition-colors truncate"
                >
                    WealthHarbor
                </Link>
            </div>

            <div className="flex-1 hidden lg:flex justify-center px-2 min-w-0">
                <div className="text-[10px] sm:text-sm font-medium text-indigo-900/60 text-center line-clamp-1">
                    Welcome,{' '}
                    <span className="font-bold text-indigo-600 truncate max-w-[60px] sm:max-w-none inline-block align-bottom">
                        {user?.username || 'User'}
                    </span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0 min-w-0">
                <NavLink
                    to="/about"
                    className={({ isActive }) =>
                        `p-1.5 md:p-2 rounded-xl transition-all shrink-0 ${isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-indigo-400 hover:bg-indigo-50 hover:text-indigo-600'}`
                    }
                    title="About Project"
                >
                ...
                <NavLink to="/profile" title="Profile" className="shrink-0">
